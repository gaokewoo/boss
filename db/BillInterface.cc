...
{
    type=SELECT_ALL;
    setSQL("SELECT PAYMENT_ID,ACCT_ID,SERV_ID,OP_CODE,STAFF_ID,PAYMENT_METHOD,TO_CHAR(PAYMENT_DATE,'YYYYMMDDHH24MISS'),ONLINE_FLAG,STATE,TO_CHAR(STATE_DATE,'YYYYMMDDHH24MISS'),NVL(OP_NOTE,0),NVL(BILL_BEG_YM,0),NVL(BILL_END_YM,0),NVL(BILL_MONTH_FLAG,0),NVL(SERV_PAY_FLAG,0) FROM BILL_INTERFACE");
    executeQuery();
